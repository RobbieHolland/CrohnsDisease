...
class GridAttentionBlock():
    ...
    def __call__(self, f, g):
        ...
        # Logging
        image_summary('feature', f[0][0][f.shape[2] // 3])
        image_summary('gate', g[0][0][g.shape[2] // 3])

        tf.summary.scalar('max_diff_attention', tf.math.reduce_max(normalised_attention) - tf.math.reduce_min(normalised_attention))
        image_summary('normalised_attenton', normalised_attention[0][0][normalised_attention.shape[2] // 3])
        image_summary('attended_attention', attended_attention[0][0][attended_attention.shape[2] // 3])

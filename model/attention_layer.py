...
class GridAttentionBlock():
    ...
    def __call__(self, f, g):
        ...
        a_m = normalised_attention[0][0][normalised_attention.shape[2] // 3]
        ...
        im = tf.subtract(im, tf.reduce_min(im))
        im = tf.divide(im, tf.reduce_max(im)) * 255
        tf.summary.image('attention map', im, max_outputs=10)

        attended_attention = tf.math.multiply(normalised_attention, mapped_f)
        a_m = attended_attention[0][0][normalised_attention.shape[2] // 3]
